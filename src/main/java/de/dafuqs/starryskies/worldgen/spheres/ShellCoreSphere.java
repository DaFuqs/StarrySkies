...
public class ShellCoreSphere extends Sphere<ShellCoreSphere.Config> {
	...
	@Override
	public PlacedSphere<?> generate(ConfiguredSphere<? extends Sphere<ShellCoreSphere.Config>, Config> configuredSphere, Config config, WorldgenRandom random, WorldGenLevel level, BlockPos pos, float radius) {
		return new ShellCoreSphere.Placed(configuredSphere, radius, configuredSphere.getDecorators(random), configuredSphere.getSpawns(random), random,
				config.mainBlock.getForSphere(level, random, pos), config.shellBlock.getForSphere(level, random, pos), config.shellThickness.sample(random), config.coreBlock.getForSphere(level, random, pos),
	...
	public static class Placed extends PlacedSphere<ShellCoreSphere.Config> {
        ...
        public Placed(ConfiguredSphere<? extends Sphere<ShellCoreSphere.Config>, ShellCoreSphere.Config> configuredSphere, float radius, List<Holder<ConfiguredSphereDecorator<?, ?>>> decorators, List<Pair<EntityType<?>, Integer>> spawns, WorldgenRandom random,
