...
public class FluidCoreSphere extends Sphere<FluidCoreSphere.Config> {
	...
	@Override
	public PlacedSphere<?> generate(ConfiguredSphere<? extends Sphere<FluidCoreSphere.Config>, Config> configuredSphere, Config config, ChunkRandom random, DynamicRegistryManager registryManager, BlockPos pos, float radius) {
		return new FluidCoreSphere.Placed(configuredSphere, radius, configuredSphere.getDecorators(random), configuredSphere.getSpawns(random), random,
				config.shellBlock.getForSphere(random, pos), config.shellThickness.get(random), config.fluidBlock, config.fillPercent.get(random), config.holeInBottomChance > random.nextFloat(), config.coreBlock.getForSphere(random, pos), config.coreRadius.get(random));
	}
